...
public sealed class SimulationTests
{
    ...
    public async Task Reports_are_created_and_contain_required_lanes()
    {
        ...
        try
        {
            ...
            Assert.Contains("Breaker state", html);
            Assert.Contains("diamond = half-open probe", html);
            Assert.Contains("class='Probe'", html);
            Assert.Contains("Baseline received", html);
            Assert.Contains("Baseline attempts", html);
            Assert.Contains("Baseline successes", html);
            Assert.Contains("Baseline failures", html);
            Assert.Contains("Protected received", html);
            Assert.Contains("Protected attempts", html);
            Assert.Contains("Protected successes", html);
            Assert.Contains("Protected failures", html);
            Assert.Contains("Rejected requests", html);
            Assert.Contains("Breaker openings", html);
            Assert.Contains("Half-open probes", html);
            Assert.Contains("Successful probes", html);
            Assert.Contains("Failed probes", html);
            Assert.Contains("Total attempts avoided", html);
